...
public class Tank {
	...
	private int x = 50,y = 50;//每次绘制的横纵坐标
	...
	//private int step = r.nextInt(10) + 55;//产生随机数模拟敌方坦克的移动
	...
	static {
		tankImage = new Image[] {
				tk.getImage(("pic/tankdown.jpg" )),
				tk.getImage(("pic/tankup.jpg" )),
				tk.getImage(("pic/tankleft.jpg" )),
				tk.getImage(("pic/tankright.jpg" ))
		};
	}
...
//			break;
		...
		if(x + Tank.width > TankeClient.WIDTH) {
			...
			//x = TankeClient.WIDTH - Tank.width + 50;
		}
		
		//if(y - Tank.height < TankeClient.HEIGHT) {
		if(y + Tank.height > TankeClient.HEIGHT) {
			y = TankeClient.HEIGHT - Tank.height;
			//y = TankeClient.HEIGHT - Tank.height + 50;
		}
		...
		if(!isMine) {
			...
			if(step == 0) {//重新产生新的数据
				step = r.nextInt(12)+3;//走到0的时候重新产生一个随机数
	...
	private Bullets fire() {
		...
		int x = this.x + Tank.width/2 - Bullets.width/2;//开火的x位置
		int y = this.y +Tank.height/2 - Bullets.height/2;//开火的y位置
